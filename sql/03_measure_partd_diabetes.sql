...
-- (Part D Medication Adherence for Diabetes Medications). See SPEC.md
-- "Measure 1" for the full definition.
--
-- NOT IMPLEMENTED: ESRD and insulin-only exclusions from the real PQA spec --
-- not modeled in this synthetic dataset. See README "Not implemented".
...
hospice_2025 AS (
    -- "At any point in the measurement year" -- SPD's hospice window differs.
...
treatment_period AS (
    SELECT
        ...
        ipsd,
        period_end,
        period_end - ipsd + 1 AS days_in_treatment_period
    FROM (
        SELECT
            member_id,
            ipsd,
...
SELECT
    ...
    tp.ipsd,
    tp.period_end,
    tp.days_in_treatment_period,
    COALESCE(c.covered_days, 0) AS covered_days,
    COALESCE(c.covered_days, 0)::DOUBLE / tp.days_in_treatment_period AS pdc,
    (COALESCE(c.covered_days, 0)::DOUBLE / tp.days_in_treatment_period) >= 0.80 AS adherent
FROM treatment_period tp
